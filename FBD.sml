...
(*------------------------------------------------------------*)   
(* Probability of CLASS III Failure Exponential Distribution  *)
(*------------------------------------------------------------*)


val PROB_OUTCOME_CLASS_III_BWR_EXP_DISTRIBUTION  = store_thm("PROB_OUTCOME_CLASS_III_BWR_EXP_DISTRIBUTION",
``!p l m s T C Y E I FR_l FR_m FR_s FR_T FR_C FR_Y FR_E FR_I t.
    prob_space p /\ 0 <= t /\
    EXP_ET_DISTRIB p l FR_l /\ EXP_ET_DISTRIB p m FR_m /\ EXP_ET_DISTRIB p s FR_s /\ EXP_ET_DISTRIB p T FR_T
 /\ EXP_ET_DISTRIB p C FR_C  /\ EXP_ET_DISTRIB p Y FR_Y  /\ EXP_ET_DISTRIB p E FR_E  /\ EXP_ET_DISTRIB p I FR_I
 /\ disjoint
      (set
         (MAP (λa. PATH p a)
           [[↓ p E t; ↓ p Y t; ↑ p C t; ↓ p l t];
	    [↓ p E t ; ↓ p Y t;  ↑ p C t; ↓ p m t];
	    [↓ p E t; ↓ p Y t;  ↑ p C t; ↓ p s t];
            [↓ p E t; ↓ p Y t;  ↑ p C t; ↓ p T t];
	    [↓ p I t;  ↑ p E t ; ↓ p Y t;  ↑ p C t; ↓ p l t];
	    [↓ p I t;  ↑ p E t ; ↓ p Y t;  ↑ p C t; ↓ p m t];
            [↓ p I t;  ↑ p E t ; ↓ p Y t;  ↑ p C t; ↓ p s t];
	    [↓ p I t;  ↑ p E t ; ↓ p Y t;  ↑ p C t; ↓ p T t]])) /\
    ALL_DISTINCT
      (MAP (λa. PATH p a)
         [[↓ p E t; ↓ p Y t;  ↑ p C t; ↓ p l t];
	  [↓ p E t; ↓ p Y t;  ↑ p C t; ↓ p m t];
    ...
    (∀x'.
		...
		↓ p I t;  ↑ p E t ; ↓ p Y t;  ↑ p C t; ↓ p m t;
		↓ p I t;  ↑ p E t ; ↓ p Y t;  ↑ p C t; ↓ p s t;
		↓ p I t;  ↑ p E t ; ↓ p Y t;  ↑ p C t; ↓ p T t;  ↓ p C t] ⇒ x' ∈ events p) /\
    (MUTUAL_INDEP p
      [↓ p E t; ↓ p Y t;  ↑ p C t; ↓ p l t;
       ↓ p E t; ↓ p Y t;  ↑ p C t; ↓ p m t;
       ↓ p E t; ↓ p Y t;  ↑ p C t; ↓ p s t;
       ...
       ↓ p I t;  ↑ p E t; ↓ p Y t;  ↑ p C t; ↓ p T t])
  ==>
  prob p (OUTCOME_CLASS_III_BWR [0:num; 1:num; 2:num; 3:num] [4:num; 5:num; 6:num; 7:num]
                                [↓ p l t; ↓ p m t; ↓ p s t; ↓ p T t]
     	  			[↑ p C t; ↓ p C t]
				[↑ p Y t; ↓ p Y t]
				[↑ p E t; ↓ p E t]
				[↑ p I t; ↓ p I t]) =
	 (1 - exp (-FR_E * t)) * (1 - exp (-FR_Y * t)) * exp (-FR_C * t) * (1 - exp (-FR_l * t)) +
         (1 - exp (-FR_E * t)) * (1 - exp (-FR_Y * t)) * exp (-FR_C * t) * (1 - exp (-FR_m * t)) +
         (1 - exp (-FR_E * t)) * (1 - exp (-FR_Y * t)) * exp (-FR_C * t) * (1 - exp (-FR_s * t))  +
         (1 - exp (-FR_E * t)) * (1 - exp (-FR_Y * t)) * exp (-FR_C * t) * (1 - exp (-FR_T * t)) +
   (1 - exp (-FR_I * t)) * exp (-FR_E * t) * (1 - exp (-FR_Y * t)) * exp (-FR_C * t) * (1 - exp (-FR_l * t)) +
   (1 - exp (-FR_I * t)) * exp (-FR_E * t) * (1 - exp (-FR_Y * t)) * exp (-FR_C * t) * (1 - exp (-FR_m * t)) +
   (1 - exp (-FR_I * t)) * exp (-FR_E * t) * (1 - exp (-FR_Y * t)) * exp (-FR_C * t) * (1 - exp (-FR_s * t)) +
   (1 - exp (-FR_I * t)) * exp (-FR_E * t) * (1 - exp (-FR_Y * t)) * exp (-FR_C * t) * (1 - exp (-FR_T * t))``,
